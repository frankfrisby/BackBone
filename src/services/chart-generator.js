...
import { uploadBuffer } from "./firebase-storage.js";
  ...
  try {
    ...
    if (!tickerData || tickerData.length === 0) {
      ...
      try {
        const fs = await import("fs");
        const path = await import("path");
        const cachePath = path.default.join(process.cwd(), "data", "tickers-cache.json");
  ...
  try {
    ...
    if (!scores) {
      try {
        const fs = await import("fs");
        const path = await import("path");
        const scoresPath = path.default.join(process.cwd(), "data", "life-scores.json");
