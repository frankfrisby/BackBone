...
const LEGACY_CACHE_PATH = dataFile(".firebase-remote-config.json");
try {
...
/**
 * Load config from local JSON file fallback (for MCP child processes that can't access Firebase)
 * File pattern: data/<configName>.json (e.g., data/config_vapi.json)
 */
const loadLocalConfigFallback = (configName) => {
  try {
    const filePath = dataFile(`${configName}.json`);
...
/**
 * Save config to local JSON file (called after successful Firebase fetch)
 * This ensures MCP child processes can access config even without Firebase
 */
const saveLocalConfigFallback = (configName, config) => {
  try {
    const filePath = dataFile(`${configName}.json`);
...
/**
 * Generic fetch config helper — Firebase → local file → env vars
 * When Firebase succeeds, saves to local file for MCP child process access
 */
const fetchConfigWithFallback = async (configName, envFallback = {}) => {
  ...
  // Tier 1: Fetch from Firebase
  const config = await fetchFirestoreDoc("config", configName);

  if (config && Object.keys(config).length > 0) {
    configCache = { ...configCache, [configName]: config };
    lastFetchTime = Date.now();
    // Save locally so MCP child processes can access it
    saveLocalConfigFallback(configName, config);
    return config;
  }

  // Tier 2: Local file fallback (for MCP child processes)
  const localConfig = loadLocalConfigFallback(configName);
  if (localConfig) {
    configCache = { ...configCache, [configName]: localConfig };
    lastFetchTime = Date.now();
    return localConfig;
  }

  // Tier 3: Fall back to env variables
  return envFallback;
};
