...
// Also back up specific data files (relative name → local path)
const BACKUP_FILES = [
  { name: "data/goals.json", local: dataFile("goals.json") },
  { name: "data/trades-log.json", local: dataFile("trades-log.json") },
  { name: "data/life-scores.json", local: dataFile("life-scores.json") },
  { name: "data/user-settings.json", local: dataFile("user-settings.json") },
  { name: "data/activity-log.json", local: dataFile("activity-log.json") },
];
...
function collectBackupFiles() {
  ...
  for (const { name, local: fullPath } of BACKUP_FILES) {
    if (fs.existsSync(fullPath)) {
      files.push({ local: fullPath, remote: `backbone/${name}`, mtime: fs.statSync(fullPath).mtimeMs });
    }
  }
...
export async function restoreFromFirebase({ overwrite = false } = {}) {
  ...
  for (const rf of remoteFiles) {
    ...
    const localPath = path.join(getBackboneHome(), relativePath);
