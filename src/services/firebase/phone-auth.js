...
import { getRealtimeMessaging } from "../messaging/realtime-messaging.js";
...
export const verifyPhoneCode = async (userId, code) => {
  ...
  if (verification.code === code.trim()) {
    ...
    // Sync phone to user-settings.json so all services can find it
    try {
      const settingsPath = path.join(DATA_DIR, "user-settings.json");
      if (fs.existsSync(settingsPath)) {
        const settings = JSON.parse(fs.readFileSync(settingsPath, "utf-8"));
        settings.phoneNumber = data.phoneNumber;
        if (settings.connections) settings.connections.phone = true;
        fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));
        console.log(`[PhoneAuth] Phone synced to user-settings.json`);
      }
    } catch (settingsError) {
      console.warn("[PhoneAuth] Settings sync error:", settingsError.message);
    }
...
/**
 * Get any verified phone record (without needing a specific user ID)
 * Useful for checking if phone is verified when firebase-user.json is missing
 */
export const getAnyVerifiedPhoneRecord = () => {
  const allData = readData();
  for (const userId of Object.keys(allData)) {
    const record = allData[userId];
    if (record?.verification?.status === VERIFICATION_STATUS.VERIFIED ||
