...
import { getDataDir, getMemoryDir, getProjectsDir, getBackboneHome } from "../paths.js";
...
export async function evaluatePlan(goal) {
  ...
  const planFile = path.join(getBackboneHome(), goal.plan.planFile);
...
export async function updatePlan(goal, changes = {}) {
  ...
  const planFilePath = path.join(getBackboneHome(), goal.plan.planFile);
