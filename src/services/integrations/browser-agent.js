...
export class BrowserAgent extends EventEmitter {
   ...
   * Uses a staged approach: try user profile first, fall back to app-managed profile
   */
  async _launchBrowser() {
    // Try to launch without killing Chrome first
    // This allows the browser to coexist if using a different profile
    const appProfileDir = path.join(DATA_DIR, "chrome-profile");
    ...
    // Strategy 1: Try to use user's Chrome profile (best for maintaining login)
    try {
      console.log("[BrowserAgent] Attempting to launch with user Chrome profile...");
      ...
      console.log("[BrowserAgent] Successfully launched with user profile");
      return;
    } catch (err1) {
      const msg = err1?.message?.toLowerCase() || "";

      // If profile is locked, try with app-managed profile
      if (msg.includes("profile") || msg.includes("user data") || msg.includes("lock") || msg.includes("in use")) {
        console.log("[BrowserAgent] User profile locked. Trying app-managed profile...");

        try {
          this.browser = await chromium.launchPersistentContext(appProfileDir, {
            headless: false,
            channel: findChromeChannel(),
            args: [
              "--disable-blink-features=AutomationControlled",
          ...
          this.page = this.browser.pages()[0] || await this.browser.newPage();
          console.log("[BrowserAgent] Launched with app-managed profile (may need to login)");
          return;
        } catch (err2) {
          console.error("[BrowserAgent] App profile also failed:", err2.message);
        }
      }

      // Strategy 2: Try killing Chrome and using user profile
      console.log("[BrowserAgent] Trying to close Chrome and retry...");
      await this._killExistingChrome();
      await new Promise(r => setTimeout(r, 2000)); // Give time for Chrome to fully close
      ...
      try {
        ...
        this.page = this.browser.pages()[0] || await this.browser.newPage();
        console.log("[BrowserAgent] Successfully launched after closing Chrome");
        return;
      } catch (err3) {
        // Final fallback: non-persistent browser (won't have login state)
        console.log("[BrowserAgent] All persistent contexts failed. Launching fresh browser...");
        console.log("[BrowserAgent] Note: You may need to log in to websites.");
        ...
        console.log("[BrowserAgent] Launched fresh browser (login required)");
        return;
      }
    }
