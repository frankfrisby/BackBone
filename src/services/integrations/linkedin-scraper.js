...
import { dataFile, getDataDir, getScreenshotsDir } from "../paths.js";
/**
...
const CHROME_PROFILE_DIR = dataFile("chrome-profile");

// Resolve Chrome user data dir per platform (env var override available)
...
const launchLinkedInContext = async ({ userDataDir, profileDirectory, headless }) => {
  ...
  // Ensure app profile directory exists
  if (!fs.existsSync(CHROME_PROFILE_DIR)) {
    fs.mkdirSync(CHROME_PROFILE_DIR, { recursive: true });
  }

  // Strategy 1: Try user's Chrome profile first (maintains login state)
  try {
    console.log("[LinkedIn] Trying to launch with user Chrome profile...");
    const ctx = await chromium.launchPersistentContext(userDataDir, {
      ...preferred,
      timeout: 30000
    });
    console.log("[LinkedIn] Launched with user profile successfully");
    return ctx;
  } catch (error) {
    const message = error?.message || String(error);
    console.log(`[LinkedIn] User profile failed: ${message.slice(0, 100)}`);

    // Strategy 2: Try app-managed profile (separate from user's Chrome)
    if (isProfileLockedError(message) && userDataDir !== CHROME_PROFILE_DIR) {
      console.log("[LinkedIn] User profile locked. Trying app-managed profile...");
      try {
        const fallbackOptions = buildLaunchOptions({ headless, profileDirectory: "Default" });
        const ctx = await chromium.launchPersistentContext(CHROME_PROFILE_DIR, {
          ...fallbackOptions,
          timeout: 30000
        });
        console.log("[LinkedIn] Launched with app-managed profile (may need to login)");
        return ctx;
      } catch (err2) {
        console.log(`[LinkedIn] App profile also failed: ${err2.message?.slice(0, 100)}`);
      }
    }

    if (isMissingExecutableError(message)) {
      console.log("[LinkedIn] Chrome not found. Trying bundled Chromium...");
      ...
      try {
        ...
        console.log(`[LinkedIn] Bundled Chromium failed: ${err3.message?.slice(0, 100)}`);
      }
    }

    // Strategy 3: Launch fresh non-persistent browser (will need login)
    console.log("[LinkedIn] Falling back to fresh browser (you'll need to log in)...");
    try {
      ...
      // Create a context that mimics persistent behavior
      ...
      // Create a wrapper that behaves like a persistent context
      const wrapper = {
        _browser: browser,
        _context: context,
      ...
      throw new Error(`Failed to launch any browser: ${err4.message}`);
    }
  }
};
...
export const scrapeLinkedInProfile = async (options = {}) => {
  ...
  try {
    console.log("[LinkedIn] Starting profile scrape...");
    ...
    try {
      ...
      return {
        ...
        hint: "Try closing Chrome completely and running again, or check if another browser instance is using the profile."
      };
    }
    ...
    if (!browser) {
      return {
        ...
        hint: "Try closing all Chrome windows and running again."
      };
    }
    ...
    if (msg.includes("closed") || msg.includes("target closed") || msg.includes("browser")) {
      hint = "The browser was closed unexpectedly. Try closing all Chrome windows and running again.";
    } else if (msg.includes("timeout")) {
      hint = "The operation timed out. LinkedIn may be slow, or you may need to log in.";
    } else if (msg.includes("login") || msg.includes("authwall")) {
      hint = "Please log in to LinkedIn when the browser opens.";
    } else if (msg.includes("profile") && msg.includes("lock")) {
      hint = "Chrome profile is locked. Close all Chrome windows and try again.";
    }
