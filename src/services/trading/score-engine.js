 ...
 *
 * BASE FORMULA (Feb 2026):
 * BaseScore = (Technical + Prediction + MarketHealth) / 3
 * Where MarketHealth = (10 - RecessionScore)
 *
 * Components:
 * - Technical Score (0-10): RSI, volatility indicators
 * - Prediction Score (0-10): AI research-based assessment
 * - Market Health (0-10): (10 - recession), where 10=booming, 0=full recession
 * - Sector Adjustment: Defensive up in recession, cyclical/speculative down
...
// Import recession score for market health calculation
 ...
 * Main scoring formula using new base: (tech + pred + marketHealth) / 3
 *
 * Base Formula: (Technical + Prediction + MarketHealth) / 3
 * Where MarketHealth = (10 - RecessionScore)
 *
 * Then: + sectorAdj + psych + dir + pos - decay + macd + volume
 *       + (pricePos × 1.25) + (earnings × 2.0) + movementPenalty
 */
export const calculateEffectiveScore = (inputs) => {
  ...
  const {
    technicalScore = 5.0,
    predictionScore = 5.0,  // Default 5 = neutral
    symbol = null,
    macdAdjustment = 0,
    volumeScore = 0,
  ...
  // Get recession score and market health
  let recessionData = { score: 5.0 };
  try {
    recessionData = getRecessionScore();
  } catch { /* use default */ }
  const recessionScore = recessionData.score ?? 5.0;
  const marketHealth = 10 - recessionScore;

  // Get sector-specific adjustment
  ...
  const baseScore = (adjTechnical + adjPrediction + marketHealth) / 3;
