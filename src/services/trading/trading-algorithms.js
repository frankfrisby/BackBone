 ...
 *
 * SCORING FORMULA (Feb 2026):
 * Base Score = (Technical + Prediction + MarketHealth) / 3
 * Where:
 *   - Technical (0-10): RSI, MACD, price position indicators
 *   - Prediction (0-10): AI research-based assessment
 *   - MarketHealth (0-10): (10 - RecessionScore), where recession 0=booming, 10=full recession
 *
 * Then sector-specific adjustments and other modifiers apply on top.
 */
 ...
 *
 * Base Formula: (Technical + Prediction + MarketHealth) / 3
 * Where MarketHealth = (10 - RecessionScore)
 *
 * Then adjustments: + sectorAdj + psych + dir + pos - decay + macd + volume
 *                   + (pricePos x 1.25) + (earnings x 2.0) + movementPenalty
 *
 * This creates a balanced score where:
 * - Technical (0-10): RSI, MACD, price position indicators
 * - Prediction (0-10): AI research-based assessment (is oversold real or fake?)
 * - MarketHealth (0-10): 10=booming market, 0=full recession
 * - SectorAdj: Defensive stocks up in recession, cyclical/speculative down
 *
 * @param {Object} inputs - Score calculation inputs (include symbol for sector adj)
...
export function calculateEffectiveScore(inputs) {
  ...
  const predictionScore = sanitize(inputs.predictionScore, 5.0); // Default 5 = neutral
  ...
  const symbol = inputs.symbol || null;

  // STEP 1: Get recession score and market health
  // RecessionScore 0 = booming market, 10 = full recession
  // MarketHealth = (10 - recession) so 10 = booming, 0 = recession
  let recessionData = { score: 5.0 };
  try {
    recessionData = getRecessionScore();
  } catch { /* use default */ }
  const recessionScore = sanitize(recessionData.score, 5.0);
  const marketHealth = 10 - recessionScore;

  // STEP 2: Get sector-specific recession adjustment
  // Defensive/inverse stocks get boost in recession, cyclical/speculative get penalty
  let sectorAdjustment = 0;
  if (symbol) {
    try {
      sectorAdjustment = getRecessionAdjustment(symbol, recessionScore);
    } catch { /* ignore */ }
  }

  // STEP 3: Adjust base scores (technical 0 becomes default)
  const adjustedTechnicalScore = technicalScore === 0 ? 6.0 : technicalScore;
  const adjustedPredictionScore = predictionScore;

  // STEP 4: Psychological adjustment
  const psychologicalAdjustment = calculatePsychologicalAdjustment(percentChange);

  // STEP 5: Historical bonuses (capped at ±1.0)
  ...
  // STEP 10: Earnings score
  const earningsScore = calculateEarningsScore(inputs.earningsDate ?? null);

  // STEP 11: Price movement penalty
  const priceMovementPenalty = calculatePriceMovementPenalty(percentChange);

  // STEP 12: Final calculation
  // NEW FORMULA: (Technical + Prediction + MarketHealth) / 3
  // This creates a balanced base where all three factors contribute equally:
  // - Technical: How the stock looks technically (0-10)
  // - Prediction: AI research-based assessment (0-10)
  // - MarketHealth: (10 - RecessionScore), so 10=booming, 0=recession
  // Then sector adjustment and other modifiers apply on top
  const baseScore = (adjustedTechnicalScore + adjustedPredictionScore + marketHealth) / 3;

  const effectiveScore = Math.max(0, Math.min(10,
    baseScore +
    sectorAdjustment +          // Sector-specific recession adjustment
  ...
  return {
    ...
    daysOld,
    effectiveScore,
    formula: `(${adjustedTechnicalScore.toFixed(1)} + ${adjustedPredictionScore.toFixed(1)} + ${marketHealth.toFixed(1)}) / 3 = ${baseScore.toFixed(2)} + sector(${sectorAdjustment.toFixed(2)}) + adj = ${effectiveScore.toFixed(2)}`
  };
}
